...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt_insert_order->bind_param("dddiidsd", $empty, $empty, $empty, $user_id, $customer_id, $empty,$Retune, $cash_received);
        ...
        // Insert order items
        $stmt = $conn->prepare("INSERT INTO order_items (order_id, product_id,title, quantity, price, applied_discount,cost) VALUES (?, ? ,? , ?, ?, ?,?)");
        ...
        foreach ($data['order_details'] as $item) {
            ...
            $stmt->bind_param("iisiddd", $order_id,$product_id,$pn, $empty,$empty, $empty,$empty);
            $stmt->execute();
        }
