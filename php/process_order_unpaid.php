...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $happy_discount = floatval($data['happy_discount'] ?? 0);
    ...
    try {
        ...
        // Insert order
        $stmt_insert_order = $conn->prepare("INSERT INTO orders (subtotal, discount, happy_discount, user_id, customer_id, total, payment_method, cash_received,old_dis) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)");
        $stmt_insert_order->bind_param("dddiidsds", $subtotal, $discount, $happy_discount, $user_id, $customer_id, $total, $payment_method, $cash_received,$oldbdiscount);
        ...
        foreach ($data['order_details'] as $item) {
           ...
           $pn = trim($item['title']);
            ...
            $product_id = $res_prod->fetch_assoc()['product_id'];
